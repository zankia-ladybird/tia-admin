	...
	<body>
		...
		<d:grid id="userGrid" title="用户列表" selectFirstRow="false" height="$D.getFullHeight('userGrid')" model="sys/user" isCellEditable="isCellEditable" queryForm="queryForm" autoQuery="true" >
			<d:toolBar>
				...
				<d:gridButton type="save" beforeAction="verify"/>
			</d:toolBar>
			<d:columns>
				...
				<d:column name="role_id" required="true" editor="combo" options="roleStore" displayField="role_name" valueField="role_id" prompt="角色" width="140" />
		...
		<script type="text/javascript">
			...
			//校验用户输入的数据
			function verify(){
				var arr = userGrid.getSelectionsModifiedData(true);
				console.log("verify has work")
				for(var i=0;i<arr.length;i++){
					//校验用户是否完整填写数据
					if(isNull(arr[i].user_code)||isNull(arr[i].user_name)||isNull(arr[i].password)){
						$D.showError("用户编号、用户名和密码不能包含空格！");
						return false;
					} 
				}
				return true;
